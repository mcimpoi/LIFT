...
def compute_keypoints(config_file, input_image_name, output_file_name, b_save_png, num_keypoint=3000):
    ...
    # read the image
    if os.path.exists(input_image_name.replace(
            "image_color", "image_gray"
    )) and "image_color" in input_image_name:
        image_gray = cv2.imread(input_image_name.replace(
            "image_color", "image_gray"
        ), 0)
        ...
        # read the image
        image_color = cv2.imread(input_image_name)
    ...
    draw_XYZS_to_img(XYZS, output_file_name + '.jpg')
    ...
    if bDraw:
        ...
        with h5py.File(output_file_name + '_scores.h5', 'w') as score_file:
    ...
    print("Saving to txt")
    saveKpListToTxt(new_kp_list, None, output_file_name)
